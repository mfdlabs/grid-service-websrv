...
INTERMEDIATE_CA_ALREADY_EXISTS="Intermediate CA already exists. If you want to overwrite it, please set the tirteenth parameter of this command to \"YES\"."
INTERMEDIATE_CA_ALREADY_EXISTS_BUT_DIFFERENT_PASSWORD="Intermediate CA already exists but with a different password. Automatically overwriting it with the new password."
...
if [ $# -eq 0 ]; then
    echo "Usage: $0 CA_NAME CA_PASSWORD PFX_PASSWORD CHAIN_NAME CHAIN_PASSWORD [IS_CHAIN_ROOT_CA=YES] [INSERT_CA_INTO_TRUSTED_CERTS=NO] [DO_NOT_GENERATE_DHPARAM=NO] [KEEP_CERTIFICATE_REQUEST_FILE=NO] [EXPIRATION_IN_DAYS=4096] [KEY_LENGTH=2048] [EXTENSION_FILE_EXTENSION=.conf] [OVERWRITE=YES]"
    exit 1
fi
...
if [ -f $CA_CERT_FILE_NAME ] && [ "$OVERWRITE" = "NO" ] ;
then
	...
	if [ -f $CA_CERT_PASSWORD_FILE_NAME ] && [ "$(cat $CA_CERT_PASSWORD_FILE_NAME)" = "$CA_PASSWORD" ] ;
	then
		printf "%s\n\n" "$INTERMEDIATE_CA_ALREADY_EXISTS"
		exit 0
	fi

	printf "%s\n\n" "$INTERMEDIATE_CA_ALREADY_EXISTS_BUT_PASSWORD_MISMATCH"
fi
...
AUTHORITY_KEY_IDENTIFIER=$(grep -i authority_key_identifier $CA_CONFIG_FILE)
if [ ! -z "$AUTHORITY_KEY_IDENTIFIER" ] ;
then
  sed -i '/authority_key_identifier/d' $CA_CONFIG_FILE
fi
...
if [ ! -z "$AUTHORITY_KEY_IDENTIFIER" ] ;
then
  sed -i '/subject_key_identifier/a '"$AUTHORITY_KEY_IDENTIFIER"'' $CA_CONFIG_FILE
fi
