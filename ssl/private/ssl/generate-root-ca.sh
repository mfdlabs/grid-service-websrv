...
ROOT_CA_ALREADY_EXISTS="Root CA already exists. If you want to overwrite it, please set the tenth parameter of this command to \"YES\"."
ROOT_CA_ALREADY_EXISTS_BUT_DIFFERENT_PASSWORD="Root CA already exists but with a different password. Automatically overwriting it with the new password."
...
if [ $# -eq 0 ]; then
    echo "Usage: $0 ROOT_CA_NAME ROOT_CA_PASSWORD PFX_PASSWORD [INSERT_ROOT_CA_INTO_TRUSTED_CERTS=NO] [DO_NOT_GENERATE_DHPARAM=NO] [HAS_EXTENSION_FILE=NO] [EXPIRATION_IN_DAYS=4096] [KEY_LENGTH=2048] [EXTENSION_FILE_EXTENSION=.conf] [OVERWRITE=YES]"
    exit 1
fi
...
UNENCRYPTED_CA_KEY_FILE_NAME=$CA_NAME.unencrypted.key
...
if [ -f $CA_CERT_NAME ] && [ "$OVERWRITE" = "NO" ] ;
then
	...
	if [ -f $CA_PASSWORD_FILE_NAME ] && [ "$(cat $CA_PASSWORD_FILE_NAME)" = "$ROOT_CA_PASSWORD" ] ;
	then
		printf "%s\n\n" "$ROOT_CA_ALREADY_EXISTS"
		exit 0
	fi

	printf "%s\n\n" "$ROOT_CA_ALREADY_EXISTS_BUT_PASSWORD_DIFFERENT"
fi
