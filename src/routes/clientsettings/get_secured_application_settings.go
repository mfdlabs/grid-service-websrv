...
func getSecuredApplicationSettings(w http.ResponseWriter, r *http.Request) {
	...
	applicationSettings, depends, _, ok := clientSettingsProvider.GetGroup(applicationName)
	if !ok {
		w.WriteHeader(http.StatusBadRequest)
		httphelpers.WriteRobloxJSONError(w, "The application name is invalid.")
		return
